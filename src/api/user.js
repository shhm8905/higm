...
const API = axios.create({ baseURL: 'https://higm-project.herokuapp.com/users' });
...
const signUp = async (userData) => {
    const res = await API.post('/signup', userData).catch(err => console.log(err));
    res.data && localStorage.setItem('user', JSON.stringify(res.data));
    return res.data;
}

const signIn = async (userData) => {
    const res = await API.post('/signin', userData).catch(err => console.log(err));
    res.data && localStorage.setItem('user', JSON.stringify(res.data));
    return res.data;
}
